...
export default function Login() {
  ...
  const handleSubmit = () => {
    loaderHtml(true)
    post('user/admin/login',{...form}).then(res => {
      if (res.success) {
        ...
        <div className="w-full bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
          <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
            ...
            <form className="space-y-4 md:space-y-6" onSubmit={e => { e.preventDefault(); handleSubmit() }}>
              ...
              <button type="submit"
              disabled={loading}
              className="w-full text-white bg-blue-600 hover:bg-blue-700 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-primary-800">{loading?'Sign in...':'Sign in'}</button>
