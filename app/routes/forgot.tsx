...
export function meta({ }: Route.MetaArgs) {
  return [
    { title: "Forgot" },
    { name: "description", content: "Forgot" },
  ];
}
...
export default function Forgot() {
  ...
  const handleSubmit = () => {
    ...
    post('user/admin/forgot/password',payload).then(res => {
      if (res.success) {
        navigate('/login')
      }
      loaderHtml(false)
          ...
          <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
            ...
            <form className="space-y-4 md:space-y-6" onSubmit={e => { e.preventDefault(); handleSubmit() }}>
              ...
              <p className="text-sm font-light text-gray-500 dark:text-gray-400">
                Just Remember? <Link to="/login" className="font-medium text-primary-600 hover:underline dark:text-primary-500">Log in</Link>
              </p>
            </form>
          </div>
        </div>
